...
public class World {
		...
		try {
			...
			for(int xx = 0; xx < map.getWidth(); xx++){
				for(int yy = 0; yy < map.getHeight(); yy++){
					...
					}else if(pixelAtual == 0xFFFFB432) {
						//Instanciar inimigo 2
						Enemy enemy2 = new Enemy(xx*16, yy*16, 16, 16,1, Entity.ENEMY2);
						Game.entities.add(enemy2);
						
					}else if(pixelAtual == 0xFFFAFF72) {
						//Maçã para o player
	...
	public static void restartGame(){
		Game.player = new Player(0,0,16,16,1.5,Game.spritesheet.getSprite(32, 0,16,16));
		Game.entities.clear();
